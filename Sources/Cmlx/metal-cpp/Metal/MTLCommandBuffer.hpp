...
{
    struct HandlerFunctionBox
    {
        std::mutex           mutex;
        MTL::HandlerFunction function;

        explicit HandlerFunctionBox(const MTL::HandlerFunction& handlerFunction) : function(handlerFunction) {}
    };
    auto blockFunction = new HandlerFunctionBox(function);
    ...
    addCompletedHandler(^(MTL::CommandBuffer* pCommandBuffer) {
        std::unique_ptr<HandlerFunctionBox> ownedFunction(blockFunction);
...
_MTL_INLINE void MTL::CommandBuffer::addScheduledHandler(const MTL::HandlerFunction& function)
{
    ...
    auto blockFunction = new HandlerFunctionBox(function);
    // Publish the copied std::function before Metal invokes the block on its scheduling thread.
    {
        std::lock_guard<std::mutex> lock(blockFunction->mutex);
    }
