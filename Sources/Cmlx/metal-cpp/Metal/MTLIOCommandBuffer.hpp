...
{
    struct IOCommandBufferHandlerFunctionBox
    {
        std::mutex                          mutex;
        MTL::IOCommandBufferHandlerFunction function;

        explicit IOCommandBufferHandlerFunctionBox(const MTL::IOCommandBufferHandlerFunction& handlerFunction) : function(handlerFunction) {}
    };
    auto blockFunction = new IOCommandBufferHandlerFunctionBox(function);
    // Publish the copied std::function before Metal invokes the block on its completion thread.
    {
        std::lock_guard<std::mutex> lock(blockFunction->mutex);
    }
    addCompletedHandler(^(MTL::IOCommandBuffer* pCommandBuffer) {
        std::unique_ptr<IOCommandBufferHandlerFunctionBox> ownedFunction(blockFunction);
        std::lock_guard<std::mutex> lock(ownedFunction->mutex);
        ownedFunction->function(pCommandBuffer);
    });
}
