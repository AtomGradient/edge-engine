...
MTL::Library* load_default_library(MTL::Device* device) {
  ...
  if (const char* override_path =
          std::getenv("EDGE_CMLX_DEFAULT_METALLIB_PATH");
      override_path != nullptr && override_path[0] != '\0') {
    std::tie(lib, error[0]) = load_library_from_path(device, override_path);
    if (lib) {
      return lib;
    }
  }
  ...
  std::tie(lib, error[1]) = load_colocated_library(device, "mlx");
  if (lib) {
    return lib;
  }
...
#ifdef SWIFTPM_BUNDLE
  std::tie(lib, error[3]) = load_colocated_library(
      device, std::string(SWIFTPM_BUNDLE) + ".bundle/default");
  if (lib) {
    return lib;
  }

  std::tie(lib, error[4]) = load_colocated_library(
      device, "../" + std::string(SWIFTPM_BUNDLE) + ".bundle/default");
  if (lib) {
    return lib;
  }
  ...
  std::tie(lib, error[7]) = load_colocated_library(
      device, "../../../../" + std::string(SWIFTPM_BUNDLE) + ".bundle/default");
  if (lib) {
    return lib;
  }
...
#ifdef SWIFTPM_BUNDLE_ALT
  std::tie(lib, error[8]) = load_colocated_library(
      device, std::string(SWIFTPM_BUNDLE_ALT) + ".bundle/default");
  if (lib) {
    return lib;
  }

  std::tie(lib, error[9]) = load_colocated_library(
      device, "../" + std::string(SWIFTPM_BUNDLE_ALT) + ".bundle/default");
  if (lib) {
    return lib;
  }

  std::tie(lib, error[10]) = load_colocated_library(
      device, "../../" + std::string(SWIFTPM_BUNDLE_ALT) + ".bundle/default");
  if (lib) {
    return lib;
  }

  std::tie(lib, error[11]) = load_colocated_library(
      device, "../../../" + std::string(SWIFTPM_BUNDLE_ALT) + ".bundle/default");
  if (lib) {
    return lib;
  }

  std::tie(lib, error[12]) = load_colocated_library(
      device,
      "../../../../" + std::string(SWIFTPM_BUNDLE_ALT) + ".bundle/default");
  if (lib) {
    return lib;
  }
#endif
  ...
  std::tie(lib, error[13]) = load_swiftpm_library(device, "default");
  if (lib) {
    return lib;
  }
  ...
  // dynamic framework.
  std::tie(lib, error[14]) = load_colocated_library(device, "Resources/default");
  if (lib) {
    return lib;
  }

  // Finally try default_mtllib_path
  std::tie(lib, error[15]) = load_library_from_path(device, default_mtllib_path);
  if (!lib) {
...
void CommandEncoder::dispatch_threadgroups(
  ...
  maybeInsertBarrier();
  buffer_ops_++;
  command_buffer_has_work_ = true;
  edge_metal_profile_record_dispatch();
...
void CommandEncoder::end_encoding() {
  ...
  auto output_fence_state = output_fence_state_;
  {
    std::lock_guard lk(output_fence_state->mutex);
    for (auto& in : all_inputs_) {
      if (auto it = output_fence_state->prev_ce_outputs.find(in);
          it != output_fence_state->prev_ce_outputs.end()) {
    ...
    for (auto& out : all_outputs_) {
      output_fence_state->prev_ce_outputs[out] = fence_;
    }
  }

  encoder_->updateFence(fence_.get());
  buffer_->addCompletedHandler([output_fence_state,
    ...
    for (auto& o : all_outputs) {
      if (auto it = output_fence_state->prev_ce_outputs.find(o);
          it != output_fence_state->prev_ce_outputs.end()) {
        if (it->second == fence) {
          output_fence_state->prev_ce_outputs.erase(it);
        }
      }
    }
  });
...
void CommandEncoder::mark_command_buffer_work() {
  command_buffer_has_work_ = true;
}

bool CommandEncoder::commit() {
  if (!command_buffer_has_work_) {
    edge_metal_profile_record_skipped_empty_commit();
    return false;
  }
  const int committed_ops = buffer_ops_;
  const size_t committed_bytes = buffer_sizes_;
  const uint64_t profile_sequence =
      edge_metal_profile_record_command_buffer_commit(
          committed_ops,
          committed_bytes);
  if (profile_sequence != 0) {
    const auto committed_at = std::chrono::steady_clock::now();
    buffer_->addCompletedHandler(
        [profile_sequence, committed_ops, committed_bytes, committed_at](
            MTL::CommandBuffer* cbuf) {
          const auto elapsed_ms = std::chrono::duration<double, std::milli>(
              std::chrono::steady_clock::now() - committed_at)
              .count();
          const bool is_error =
              cbuf->status() == MTL::CommandBufferStatusError;
          const char* error_message = "";
          if (is_error && cbuf->error() != nullptr &&
              cbuf->error()->localizedDescription() != nullptr) {
            error_message =
                cbuf->error()->localizedDescription()->utf8String();
          }
          edge_metal_profile_record_command_buffer_completion(
              profile_sequence,
              committed_ops,
              committed_bytes,
              elapsed_ms,
              static_cast<int>(cbuf->status()),
              is_error,
              error_message);
        });
  } else if (edge_metal_completion_probe_enabled()) {
    buffer_->addCompletedHandler([](MTL::CommandBuffer*) {});
  }
  buffer_->commit();
  buffer_ = NS::RetainPtr(queue_->commandBufferWithUnretainedReferences());
  buffer_ops_ = 0;
  buffer_sizes_ = 0;
  command_buffer_has_work_ = false;
  return true;
}
...
Device::Device() : device_(load_device()), residency_set_(device_.get()) {
  ...
  try {
    ...
    // SwiftPM command-line builds do not always produce a default.metallib.
    // JIT kernel paths build their own libraries lazily; fail only if a caller
    // asks for a precompiled default-library kernel.
...
MTL::ComputePipelineState* Device::get_kernel(
  ...
  if (!default_library_) {
    throw std::runtime_error(
        "Default metallib is unavailable; use a JIT kernel library path for " +
        base_name);
  }
  return get_kernel(
      base_name,
