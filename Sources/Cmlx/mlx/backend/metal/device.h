...
class MLX_API CommandEncoder {
 ...
 private:
  struct OutputFenceState {
    std::unordered_map<const void*, NS::SharedPtr<MTL::Fence>> prev_ce_outputs;
    std::mutex mutex;
  };
  ...
  bool command_buffer_has_work_{false};
  ...
  // Completion handlers can run after the CommandEncoder instance starts
  // tearing down, so the fence map/mutex must outlive `this`.
  std::shared_ptr<OutputFenceState> output_fence_state_{
      std::make_shared<OutputFenceState>()};
};
...
class MLX_API Device {
 public:
  ...
  void set_max_ops_mb_per_buffer(int max_ops, int max_mb) {
    max_ops_per_buffer_ = std::max(1, max_ops);
    max_mb_per_buffer_ = std::max(1, max_mb);
  }
