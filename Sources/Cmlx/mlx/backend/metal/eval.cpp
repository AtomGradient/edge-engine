...
void eval(array& arr) {
  ...
  auto outputs = arr.outputs();
  {
    ...
    const char* primitive_name = arr.primitive().name();
    auto profile_start = std::chrono::steady_clock::time_point{};
    const bool profile_enabled = metal::edge_metal_profile_enabled();
    if (profile_enabled) {
      profile_start = std::chrono::steady_clock::now();
    }
    metal::EdgeMetalProfilePrimitiveScope profile_scope(primitive_name);
    ...
    if (profile_enabled) {
      const auto elapsed_ms = std::chrono::duration<double, std::milli>(
          ...
          .count();
      metal::edge_metal_profile_record_primitive_eval(
          primitive_name,
          elapsed_ms);
    }
  }
