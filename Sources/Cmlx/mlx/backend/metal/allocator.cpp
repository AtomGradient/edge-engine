...
Buffer MetalAllocator::malloc(size_t size) {
  ...
  if (!buf) {
    size_t mem_required =
        get_active_memory_unlocked() + get_cache_memory_unlocked() + size;
  ...
  if (get_cache_memory_unlocked() > max_pool_size_) {
    num_resources_ -= buffer_cache_.release_cached_buffers(
        get_cache_memory_unlocked() - max_pool_size_);
  }
...
size_t MetalAllocator::get_active_memory() {
  std::unique_lock lk(mutex_);
  return get_active_memory_unlocked();
}

size_t MetalAllocator::get_peak_memory() {
  std::unique_lock lk(mutex_);
  return get_peak_memory_unlocked();
}

size_t MetalAllocator::get_cache_memory() {
  std::unique_lock lk(mutex_);
  return get_cache_memory_unlocked();
}
