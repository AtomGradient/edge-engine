...
inline const char* edge_matmul_getenv(const char* name) {
  const char* value = std::getenv(name);
  return (value && value[0] != '\0') ? value : nullptr;
}

inline const char* edge_matmul_getenv(
    const char* primary,
    const char* fallback) {
  if (const char* value = edge_matmul_getenv(primary)) {
    return value;
  }
  return edge_matmul_getenv(fallback);
}

inline bool edge_matmul_env_enabled(
    const char* primary,
    const char* fallback) {
  const char* value = edge_matmul_getenv(primary, fallback);
  if (!value) {
    return false;
  }
...
inline int edge_matmul_swizzle_log(int fallback) {
  // Diagnostic-only experiment knob.
  // review-by: 2026-06-02
  // delete unless a named follow-up experiment still needs it.
  const char* value = edge_matmul_getenv(
      "EDGE_MATMUL_NAX_SWIZZLE_LOG",
      "EDGE_CMLX_MATMUL_NAX_SWIZZLE_LOG");
  if (!value) {
    return fallback;
  }
  errno = 0;
...
inline char edge_matmul_devc_override(char fallback) {
  // Diagnostic-only experiment knob.
  // review-by: 2026-06-02
  // delete unless a named follow-up experiment still needs it.
  // Overrides the architecture class character used by
  // matmul routing. This affects GEMM macro selection, NAX tile selection, and
  // split-k routing thresholds.
  const char* value = edge_matmul_getenv(
      "EDGE_MATMUL_DEVC_OVERRIDE",
      "EDGE_CMLX_MATMUL_DEVC_OVERRIDE");
  if (!value) {
    return fallback;
  }
  if (std::strcmp(value, "large") == 0) {
    return 'd';
  }
...
inline void edge_log_matmul_nax_dispatch(
    const char* path,
    int M,
    int N,
    int K,
    ...
    int split_k,
    metal::Device& d) {
  if (!edge_matmul_env_enabled(
          "EDGE_MATMUL_DISPATCH_DIAGNOSTICS",
          "EDGE_CMLX_MATMUL_DISPATCH_DIAGNOSTICS")) {
    return;
  }
  std::fprintf(
      stderr,
      "edge_matmul_dispatch path=%s arch=%s M=%d N=%d K=%d B=%d "
      "bm=%d bn=%d bk=%d wm=%d wn=%d swizzle_log=%d split_k=%d\n",
      path,
...
void steel_matmul_regular_axpby_nax(
  ...
  char devc = edge_matmul_devc_override(d.get_architecture().back());
  ...
  swizzle_log = edge_matmul_swizzle_log(swizzle_log);
  edge_log_matmul_nax_dispatch(
      "regular_nax",
      M,
      N,
      K,
