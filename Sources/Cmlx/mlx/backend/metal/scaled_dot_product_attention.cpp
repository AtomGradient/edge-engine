...
void sdpa_vector(
    ...
    const std::optional<array>& mask,
    const std::optional<array>& sinks,
    std::optional<std::reference_wrapper<array>> scores_out = std::nullopt) {
  // Set the kernel name
  std::string kname;
  kname.reserve(64);
  kname += "sdpa_vector_";
  ...
  bool do_output_scores = scores_out.has_value();
  metal::MTLFCList func_consts = {
      ...
      {&do_output_scores, MTL::DataType::DataTypeBool, 27},
  };
  ...
  hash_name += do_output_scores ? "_scores" : "_noscores";
  ...
  if (do_output_scores) {
    compute_encoder.set_output_array(scores_out->get(), 18);
  }
...
void sdpa_vector_2pass(
  ...
  if (do_output_scores) {
    compute_encoder.set_output_array(scores_out->get(), 19);
  }
...
void sdpa_vector_quantized(
    const Stream& s,
    metal::Device& d,
    const array& q,
    const array& k_weights,
    const array& k_scales,
    const array& k_biases,
    const array& v_weights,
    ...
    const std::optional<array>& sinks,
    std::optional<std::reference_wrapper<array>> scores_out = std::nullopt) {
  // Kernel name: sdpa_vector_quantized_<type>_<D>_<V>_<bits>bit_gs<group_size>
  std::string kname;
  kname.reserve(96);
  ...
  kname += std::to_string(q.shape(-1));
  kname += "_";
  kname += std::to_string(q.shape(-1));  // V = D
  kname += "_";
  ...
  // Compute sizes
  int gqa_factor = q.shape(1) / k_weights.shape(1);
  int N = k_weights.shape(2);

  // Head strides for quantized data
  size_t k_head_stride =
      k_weights.shape(1) == 1 ? k_weights.strides(0) : k_weights.strides(1);
  size_t v_head_stride =
      v_weights.shape(1) == 1 ? v_weights.strides(0) : v_weights.strides(1);
  ...
  // No mask support in quantized kernel (DSR generation phase doesn't need it)
  bool has_mask = false;
  ...
  // Set arguments
  compute_encoder.set_input_array(q, 0);
  compute_encoder.set_input_array(k_weights, 1);
  compute_encoder.set_input_array(k_scales, 2);
  compute_encoder.set_input_array(k_biases, 3);
  compute_encoder.set_input_array(v_weights, 4);
  ...
  compute_encoder.set_bytes(ks_head_stride, 12);
  compute_encoder.set_bytes(vs_head_stride, 13);
  compute_encoder.set_bytes(scale, 14);
  if (has_sinks) {
    compute_encoder.set_input_array(*sinks, 15);
    compute_encoder.set_bytes(q.shape(1), 16);
  }
  if (do_output_scores) {
    compute_encoder.set_output_array(scores_out->get(), 17);
  }

  compute_encoder.dispatch_threadgroups(grid_dims, group_dims);
}

void sdpa_vector_2pass_quantized(
    const Stream& s,
    metal::Device& d,
    const array& q,
    const array& k_weights,
  ...
  // Block selection (same heuristics as original)
  char devc = d.get_architecture().back();
  int blocks;
  if (devc == 's') {
    blocks = 64;
  ...
  MTL::Size group_dims_1(32, gqa_factor, q.shape(2));
  MTL::Size grid_dims_1(k_weights.shape(1), q.shape(0), blocks);
  ...
  array intermediate(intermediate_shape, q.dtype(), nullptr, {});
  intermediate_shape.pop_back();
  array sums_arr(intermediate_shape, float32, nullptr, {});
  array maxs_arr(std::move(intermediate_shape), float32, nullptr, {});
  intermediate.set_data(allocator::malloc(intermediate.nbytes()));
  sums_arr.set_data(allocator::malloc(sums_arr.nbytes()));
  maxs_arr.set_data(allocator::malloc(maxs_arr.nbytes()));
  auto& compute_encoder = metal::get_command_encoder(s);
  compute_encoder.add_temporary(intermediate);
  compute_encoder.add_temporary(sums_arr);
  compute_encoder.add_temporary(maxs_arr);
  ...
  // Pass 1
  auto kernel = d.get_kernel(kname, hash_name, func_consts);
  check_kernel_threadgroup_size(kernel, group_dims_1, hash_name);
  compute_encoder.set_compute_pipeline_state(kernel);
  ...
  compute_encoder.set_output_array(sums_arr, 8);
  compute_encoder.set_output_array(maxs_arr, 9);
  compute_encoder.set_bytes(N, 10);
  compute_encoder.set_bytes(k_head_stride, 11);
  compute_encoder.set_bytes(v_head_stride, 12);
  compute_encoder.set_bytes(ks_head_stride, 13);
  compute_encoder.set_bytes(vs_head_stride, 14);
  ...
  compute_encoder.dispatch_threadgroups(grid_dims_1, group_dims_1);

  // Pass 2 -- reuse original sdpa_vector_2pass_2 (no quantization needed)
  std::string kname2 = "sdpa_vector_2pass_2_";
  kname2 += get_type_string(q.dtype());
  kname2 += "_";
  kname2 += std::to_string(q.shape(-1));  // V = D

  auto kernel2 = d.get_kernel(kname2);
  compute_encoder.set_compute_pipeline_state(kernel2);

  compute_encoder.set_input_array(intermediate, 0);
  compute_encoder.set_input_array(sums_arr, 1);
  compute_encoder.set_input_array(maxs_arr, 2);
  compute_encoder.set_output_array(out, 3);
  compute_encoder.set_bytes(blocks, 4);

  MTL::Size group_dims_2(1024, 1, 1);
  MTL::Size grid_dims_2(q.shape(0) * q.shape(1), q.shape(2), 1);
  check_kernel_threadgroup_size(kernel2, group_dims_2, kname2);
  compute_encoder.dispatch_threadgroups(grid_dims_2, group_dims_2);
}
...
void ScaledDotProductAttention::eval_gpu(
  ...
  // Allocate scores output if requested
  std::optional<std::reference_wrapper<array>> scores_ref = std::nullopt;
  if (output_scores_ && outputs.size() > 1) {
    auto& scores = outputs[1];
    scores.set_data(allocator::malloc(scores.nbytes()));
    scores_ref = std::ref(scores);
  }
  ...
  if (q_pre.shape(2) <= 8) {
    ...
    if (((devc == 'd' || devc == 's') && k.shape(2) >= 1024) ||
      ...
      sdpa_vector_2pass(
          s, d, q, k, v, o, scale_, do_causal, mask, sinks, scores_ref);
    } else {
      sdpa_vector(
          s, d, q, k, v, o, scale_, do_causal, mask, sinks, scores_ref);
    }
...
void ScaledDotProductAttentionQuantized::eval_gpu(
    const std::vector<array>& inputs,
    std::vector<array>& outputs) {
  auto& s = stream();
  auto& d = metal::device(s.device);

  // inputs: q(0), k_w(1), k_s(2), k_b(3), v_w(4), v_s(5), v_b(6), [sinks(7)]
  auto& q_pre = inputs[0];
  auto& k_w = inputs[1];
  auto& k_s = inputs[2];
  auto& k_b = inputs[3];
  ...
  auto is_contiguous = [](const array& arr) {
    return arr.flags().row_contiguous ||
        (arr.strides(-1) == 1 && arr.strides(0) == arr.strides(1) * arr.shape(1));
  };
  ...
  std::optional<array> sinks = std::nullopt;
  if (has_sinks_) {
    sinks = copy_unless(is_contiguous, inputs.back());
  }

  bool q_copied = !q_copy_unless(q_pre);
  array q = q_copied ? contiguous_copy_gpu(q_pre, s) : q_pre;

  // Donate query if possible
  if (q.is_donatable() && q.flags().row_contiguous && q.size() == o.size()) {
    o.copy_shared_buffer(q);
  } else {
    if (q_copied) {
  ...
  // Route to 2-pass if sequence is long enough
  char devc = d.get_architecture().back();
  int N = k_w.shape(2);
  if (((devc == 'd' || devc == 's') && N >= 1024) ||
      (k_w.shape(1) < q.shape(1) && N >= 4096)) {
    sdpa_vector_2pass_quantized(
        s, d, q, k_w, k_s, k_b, v_w, v_s, v_b, o,
        scale_, bits_, group_size_, sinks, scores_ref);
  } else {
