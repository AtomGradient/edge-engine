...
array rms_norm_scale(
  ...
  if (x.ndim() == 0) {
    std::ostringstream msg;
    msg << "[rms_norm_scale] Input must have at least 1 dimension but got "
           "input with 0 dimensions.";
    throw std::invalid_argument(msg.str());
  }
  ...
  if (!issubdtype(out_type, floating)) {
    std::ostringstream msg;
    msg << "[rms_norm_scale] Received unsupported type " << out_type << ".";
    throw std::invalid_argument(msg.str());
  }

  auto s = to_stream(s_);
  auto fallback = [eps, scale, out_type, s](const std::vector<array>& inputs) {
    auto y = astype(inputs[0], float32, s);
    y = multiply(
        y,
        rsqrt(
            add(mean(square(y, s), -1, /* keepdims */ true, s),
                array(eps, float32),
                s),
            s),
        s);
    y = multiply(y, array(scale, float32), s);
    return std::vector<array>{astype(y, out_type, s)};
  };

  if (!RMSNormScale::use_fallback(s)) {
    return array(
        x.shape(),
        out_type,
        std::make_shared<RMSNormScale>(s, fallback, eps, scale),
        {astype(x, out_type, s)});
  }
  return fallback({x})[0];
}
...
bool RMSNormScale::is_equivalent(const Primitive& other) const {
  const RMSNormScale& a_other = static_cast<const RMSNormScale&>(other);
  return eps_ == a_other.eps_ && scale_ == a_other.scale_;
}
...
std::pair<array, array> scaled_dot_product_attention_with_scores(
    const array& queries,
    const array& keys,
    const array& values,
    const float scale,
    ...
    const std::optional<array>& sinks /* = {} */,
    StreamOrDevice s /* = {}*/) {
  // Validation: reuse the same checks as scaled_dot_product_attention
  for (const auto& tensor : {queries, keys, values}) {
    if (tensor.ndim() != 4) {
      std::ostringstream msg;
      msg << "[scaled_dot_product_attention_with_scores] input with shape "
          << tensor.shape() << " expected to be rank 4";
      throw std::invalid_argument(msg.str());
    }
  }
  ...
  if (!issubdtype(final_type, floating)) {
    throw std::invalid_argument(
        "[scaled_dot_product_attention_with_scores] Unsupported type.");
  }
  bool has_sinks = sinks.has_value();
  ...
  // Fallback lambda that also computes scores
  auto fallback = [scale,
                   n_q_heads,
                   n_kv_heads,
                   do_causal,
    ...
    auto scores = matmul(q, swapaxes(k, -1, -2, s), s);
    // Save pre-softmax scores before masking modifies them
    ...
    if (n_repeats > 1) {
      out = flatten(out, 1, 2, s);
      raw_scores = flatten(raw_scores, 1, 2, s);
    }
    return std::vector<array>{out, raw_scores};
  };

  bool is_training = detail::in_grad_tracing();
  if (!ScaledDotProductAttention::use_fallback(
          q, k, v, has_mask, has_arr_mask, do_causal, is_training,
          false /* output_logsumexp */, stream)) {
    if (has_bool_mask && !ScaledDotProductAttention::supports_bool_mask()) {
      float inf = std::numeric_limits<float>::infinity();
      array& mask = inputs[3];
      mask = where(
    ...
    Shape out_shape{q.shape(0), q.shape(1), q.shape(2), v.shape(-1)};
    Shape scores_shape{q.shape(0), q.shape(1), q.shape(2), k.shape(2)};
    auto primitive = std::make_shared<ScaledDotProductAttention>(
        stream, fallback, scale, do_causal, has_sinks,
        false /* output_logsumexp */, true /* output_scores */);
    auto results = array::make_arrays(
        {std::move(out_shape), std::move(scores_shape)},
        {final_type, float32},
        primitive,
        std::move(inputs));
  ...
  // Fallback path
  auto results = fallback(std::move(inputs));
  return {results[0], results[1]};
}

std::pair<array, array> scaled_dot_product_attention_quantized_with_scores(
    const array& queries,
  ...
  if (queries.ndim() != 4) {
    throw std::invalid_argument(
        "[scaled_dot_product_attention_quantized_with_scores] "
        "queries must be rank 4");
  }
  if (k_weights.ndim() != 4 || k_scales.ndim() != 4 || k_biases.ndim() != 4) {
    throw std::invalid_argument(
        "[scaled_dot_product_attention_quantized_with_scores] "
        "quantized key arrays must be rank 4");
  }
  if (v_weights.ndim() != 4 || v_scales.ndim() != 4 || v_biases.ndim() != 4) {
    throw std::invalid_argument(
        "[scaled_dot_product_attention_quantized_with_scores] "
        "quantized value arrays must be rank 4");
  }
  if (bits != 4 && bits != 8) {
    throw std::invalid_argument(
        "[scaled_dot_product_attention_quantized_with_scores] "
  ...
  if (head_dim != 128 && head_dim != 256) {
    throw std::invalid_argument(
        "[scaled_dot_product_attention_quantized_with_scores] "
        "only head_dim=128 and head_dim=256 are supported");
  }
  ...
  if (!issubdtype(final_type, floating)) {
    throw std::invalid_argument(
        "[scaled_dot_product_attention_quantized_with_scores] "
        "queries must be a floating point type");
  }
  ...
  auto q = queries;
  auto n_q_heads = q.shape(1);
  auto n_kv_heads = k_weights.shape(1);
  int N = k_weights.shape(2);

  // Fallback: dequantize + standard SDPA
  auto fallback = [scale, bits, group_size, n_q_heads, n_kv_heads, has_sinks, s_](
                      std::vector<array> inputs) -> std::vector<array> {
    auto& q = inputs[0];
    auto dk = dequantize(inputs[1], inputs[2], inputs[3],
                         group_size, bits, "affine", std::nullopt,
                         std::nullopt, s_);
    auto dv = dequantize(inputs[4], inputs[5], inputs[6],
                         group_size, bits, "affine", std::nullopt,
                         std::nullopt, s_);
    int n_repeats = n_q_heads / n_kv_heads;
    auto qscaled = multiply(array(scale, q.dtype()), q, s_);
    auto k = dk;
    auto v = dv;
    if (n_repeats > 1) {
      qscaled = unflatten(qscaled, 1, {n_kv_heads, n_repeats}, s_);
      k = expand_dims(k, 2, s_);
      v = expand_dims(v, 2, s_);
    }
    auto scores = matmul(qscaled, swapaxes(k, -1, -2, s_), s_);
    auto raw_scores = scores;
    scores = softmax(scores, std::vector<int>{-1}, true, s_);
    auto out = matmul(scores, v, s_);
    if (n_repeats > 1) {
      out = flatten(out, 1, 2, s_);
      raw_scores = flatten(raw_scores, 1, 2, s_);
    }
    return std::vector<array>{out, raw_scores};
  };

  std::vector<array> inputs = {
      q, k_weights, k_scales, k_biases,
      v_weights, v_scales, v_biases};
  if (has_sinks) {
    inputs.push_back(astype(*sinks, final_type, s));
  }

  // Check if we can use the fused kernel
  bool can_use_fused = (s.device == Device::gpu) &&
      (head_dim == 128 || head_dim == 256) &&
      (bits == 4 || bits == 8) &&
      (group_size == 64 || group_size == 128) &&
      (final_type == float16 || final_type == bfloat16) &&
      (q.shape(2) <= 8);  // vector mode only

  if (can_use_fused) {
    Shape out_shape{q.shape(0), q.shape(1), q.shape(2), head_dim};
    Shape scores_shape{q.shape(0), q.shape(1), q.shape(2), N};
    auto primitive =
        std::make_shared<ScaledDotProductAttentionQuantized>(
            s, fallback, scale, bits, group_size, has_sinks,
            true /* output_scores */);
    auto results = array::make_arrays(
        {std::move(out_shape), std::move(scores_shape)},
        {final_type, float32},
