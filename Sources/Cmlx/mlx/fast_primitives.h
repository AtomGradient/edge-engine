...
class RMSNormScale : public Custom {
 public:
  RMSNormScale(
      Stream stream,
      std::function<std::vector<array>(std::vector<array>)> fallback,
      float eps,
      float scale)
      : Custom(stream, std::move(fallback)), eps_(eps), scale_(scale) {}
  ...
  DEFINE_NAME(RMSNormScale)
  bool is_equivalent(const Primitive& other) const override;
  DEFINE_INPUT_OUTPUT_SHAPE()

  auto state() const {
    return std::make_pair(eps_, scale_);
  }
...
class ScaledDotProductAttentionQuantized : public Custom {
 public:
  ScaledDotProductAttentionQuantized(
      Stream stream,
      std::function<std::vector<array>(std::vector<array>)> fallback,
      float scale,
      int bits,
  ...
  DEFINE_NAME(ScaledDotProductAttentionQuantized);
  DEFINE_INPUT_OUTPUT_SHAPE()
  auto state() const {
    return std::make_tuple(
        nullptr, scale_, bits_, group_size_, has_sinks_, output_scores_);
  }
