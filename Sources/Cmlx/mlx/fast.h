...
/** Computes: O = softmax(Q @ K.T) @ V, also returns pre-softmax scores **/
MLX_API std::pair<array, array> scaled_dot_product_attention_with_scores(
    const array& queries,
    const array& keys,
    const array& values,
    const float scale,
...
/** Quantized SDPA: fused dequantize + attention for quantized KV cache **/
